...
/**
 * 테스트 환경에서 DispatcherServlet이 만드는 서블릿 컨텍스트를 자유롭게 설정할 수 있게한 코드
 */
public class ConfigurableDispatcherServlet extends DispatcherServlet {
  // 설정 방법을 확장해서 클래스와 XML 파일 두가지 방법을 모두 지원하게 만듬.
  private Class<?>[] classes;
  private String[] locations;

  // 컨트롤러가 DispatcherServlet에 돌려주는 모델과 뷰 정보를 저장해서 테스트에서 확인할 수 있게 해줌.
  ...
  /**
   * 주어진 클래스로부터 상대적인 위치의 클래스패스에 있는 설정파일을 지정할 수 있게 해줌.
   */
  ...
  /**
   * DispatcherServlet의 서블릿 컨텍스트를 생성하는 메소드를 오버라이드해서 테스트용 메타정보를 이용해 서블릿 컨텍스트를 생성하게 함.
   */
  ...
  /**
   * 뷰를 실행하는 과정을 가로채서 컨트롤러가 돌려준 ModelAndView 정보를 따로 저장해둠.
   * 테스트에서 HttpServletResponse를 확인하는 대신 컨트롤러가 리턴한 ModelAndView를 검증할 수 있게 해줌.
   */
